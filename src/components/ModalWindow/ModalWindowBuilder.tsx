...
const ModalWindowBuilder: FC<IProps> = observer( ( props ) => {
	if ( !props.provider.window ) return null;
	...
	const { variant, assessments, buttons } = props;
				...
				{content.map( ( elem, index ) => {
					...
					switch ( elem.type ) {
						...
						case 'results':
							return (
								<div
									className={styles.mwResults}
									key={'results' + index}
								>
									{props.provider.results &&
										props.provider.results.map( ( info ) => (
											<MiniInfo
												key={info.name}
												provider={info.value}
												caption={info.name}
